...
export function avatarImage(image: string, identifier: string, id: ObjectID, size: number, badge?: number) {
  if (badge) {
    return (
      <StyledBadge key={id.toString()} badgeContent={badge} overlap="circular">
        <Avatar aria-label={identifier} src={image} className={`h-${size} w-${size} rounded-full bg-obs-primary`}></Avatar>
      </StyledBadge>
    );
  }
  return <Avatar aria-label={identifier} key={id.toString()} src={image} className={`h-${size} w-${size} rounded-full bg-obs-primary`}></Avatar>;
}
